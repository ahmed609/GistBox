...
var GistAddForm = React.createClass({
    ...
    render: function render() {
        return React.createElement(
            ...
            React.createElement(
                'form',
                { classNamme: 'form-inline justify-content-center', onSubmit: this.addGist },
                React.createElement('input', { className: 'form-control', value: this.state.username, onChange: this.onChange, placeholder: 'Enter Githib Username...' }),
                React.createElement(
                    'button',
...
var GistBox = React.createClass({
    ...
    render: function render() {
        var newGist = function newGist(gist) {
            return React.createElement(_Gist2.default, { username: gist.username, url: gist.url, style: { color: 'white' } });
        };

        return React.createElement(
            'div',
            { className: 'jumbotron jumbotron-fluid', style: { backgroundImage: 'url(./img/banner-1557842_1920.jpg)', color: 'black' } },
            React.createElement(
                'div',
                { className: 'container text-center' },
                React.createElement(
                    'h1',
                    { className: 'mt-0', style: { fontSize: 65, fontFamily: 'Corben', fontWeight: '700' } },
                    'GistBox'
                ),
